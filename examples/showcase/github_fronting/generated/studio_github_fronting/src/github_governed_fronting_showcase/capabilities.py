...
DEFAULT_SERVICE_ID = "github-governance-service"
...
async def _handle_generated_capability(capability: dict[str, Any], ctx: InvocationContext, params: dict[str, Any], capability_registry: dict[str, Capability] | None = None) -> dict[str, Any]:
    ...
    if capability.get("execution_posture") == "prepare_only":
        if capability["capability_id"] == "github.issue.prepare":
            ...
            if prepared.get("execution_status") != "backend_execution_stub":
                if params.get("request_execution_approval") is True and ctx.approval_grant is None:
                    raise ANIPError(
                        "approval_required",
                        "GitHub issue creation requires an ANIP approval grant before mutation.",
    ...
    return await backend_adapter.execute(capability, plan, plan["adapter_input"])
