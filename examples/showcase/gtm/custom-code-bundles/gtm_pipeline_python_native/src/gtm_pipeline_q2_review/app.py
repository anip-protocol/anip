"""Generated Python GTM app entrypoint with native approval routes."""
...
def create_app() -> FastAPI:
    ...
    async def list_approvals(request: Request, status: str | None = None):
        ...
        if actor.get("can_approve_followup") or actor.get("can_approve_routing"):
    ...
    async def approve(approval_request_id: str, request: Request):
        actor = _actor_from_request(request)
        if not actor.get("can_approve_followup") and not actor.get("can_approve_routing"):
