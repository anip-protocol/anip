"""Start the GTM C#-native language-parity stack."""
...
DEFAULT_GENERATED_DIR = REPO_ROOT / "output" / "gtm-language-parity" / "csharp-native"
...
def _start_services(args: argparse.Namespace) -> list[subprocess.Popen[bytes]]:
    ...
    subprocess.run(
        ["dotnet", "build", "--nologo", "-v", "minimal", "/m:1", "/p:UseSharedCompilation=false"],
        cwd=generated_dir,
        check=True,
    )
    ...
    for index, (_name, service_id) in enumerate(SERVICE_SLICES):
        ...
        env.update(
            {
                "ASPNETCORE_URLS": f"http://0.0.0.0:{args.base_port + index}",
        ...
        processes.append(subprocess.Popen(["dotnet", "run", "--no-build", "--no-launch-profile"], cwd=generated_dir, env=env))
    return processes
    ...
    env.update(
        {
            ...
            "DATABASE_URL": args.database_url,
            "STUDIO_DATABASE_URL": args.studio_database_url,
...
def main() -> int:
    ...
    parser.add_argument("--base-port", type=int, default=4700)
    parser.add_argument("--runtime-port", type=int, default=9309)
    ...
    parser.add_argument("--studio-database-url", default="postgresql://anip:anip@127.0.0.1:5432/anip_studio")
