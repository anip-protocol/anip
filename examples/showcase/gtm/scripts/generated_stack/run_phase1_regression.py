...
DEFAULT_CASES = Path(__file__).resolve().parent.parent / "phase1_regression_cases.json"
DEFAULT_OUTPUT_DIR = REPO_ROOT / "docs" / "examples" / "gtm-showcase" / "regression-runs"
DEFAULT_RUNTIME_URL = "http://127.0.0.1:9304"
POSTGRES_CONTAINER = os.getenv("GTM_REGRESSION_POSTGRES_CONTAINER", "go_registry_gtm_pipeline_custom-gtm-postgres-1")
...
HTTP_TIMEOUT_SECONDS = float(os.getenv("GTM_REGRESSION_HTTP_TIMEOUT_SECONDS", "60"))
...
def main() -> int:
    ...
    for case_index, case in enumerate(suite["cases"], start=1):
        ...
        for turn_index, turn_def in enumerate(turn_defs, start=1):
            ...
            assistant_history: dict[str, Any] = {"role": "assistant", "content": _assistant_followup_text(payload)}
            if payload.get("continuation"):
                assistant_history["continuation"] = payload["continuation"]
            history.append(assistant_history)
            print(
                f"[{case_index}/{total_cases}] {case['id']} turn {turn_index}/{len(turn_defs)} "
                f"answered in {turn_duration:.2f}s",
                file=sys.stderr,
                flush=True,
            )
        case_summary = _summarize_case(case, payloads, args.runtime_url)
        case_duration = time.perf_counter() - case_started
        case_summary["duration_seconds"] = round(case_duration, 3)
        for turn_summary, turn_duration in zip(case_summary.get("turns", []), turn_durations):
            turn_summary["duration_seconds"] = round(turn_duration, 3)
        results.append(case_summary)
        status = "PASS" if case_summary["pass"] else "FAIL"
        elapsed = time.perf_counter() - suite_started
        average = elapsed / case_index
        remaining = average * (total_cases - case_index)
        print(
            f"[{case_index}/{total_cases}] {case['id']} {status} "
            f"case={case_duration:.2f}s elapsed={elapsed:.2f}s eta={remaining:.2f}s",
            file=sys.stderr,
            flush=True,
        )
