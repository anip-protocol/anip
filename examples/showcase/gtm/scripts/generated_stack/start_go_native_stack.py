"""Start the GTM Go-native language-parity stack.

This starts generator-produced Go ANIP services from
``output/gtm-language-parity/go-native``. It intentionally does not proxy to the
...
DEFAULT_GENERATED_DIR = REPO_ROOT / "output" / "gtm-language-parity" / "go-native"
...
def _start_services(args: argparse.Namespace) -> list[subprocess.Popen[bytes]]:
    ...
    for index, (_name, service_id) in enumerate(SERVICE_SLICES):
        ...
        processes.append(subprocess.Popen(["go", "run", "."], cwd=generated_dir, env=env))
    return processes
...
def main() -> int:
    ...
    parser.add_argument("--runtime-port", type=int, default=9307)
