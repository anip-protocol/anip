"""Postgres-backed enrichment data access for the generated GTM stack."""
...
DEFAULT_DATABASE_URL = os.getenv(
    "DATABASE_URL",
    "postgresql://anip:anip@localhost:5454/anip_gtm",
)
...
def get_account_enrichment_summary(account_names: str | list[str] | None, limit: int = 5) -> list[dict[str, Any]]:
    names = _parse_account_names(account_names)
    if not names:
        return []
    query = """
        select account_name, sector, office_location, parent_company, revenue_band,
               employee_band, icp_fit, intent_signal, likely_buying_motion,
...
def get_lookalike_accounts(reference_account: str, limit: int = 5) -> dict[str, Any] | None:
    reference_query = """
...
def get_at_risk_account_enrichment_summary(
    *,
    ...
    risk_query = f"""
        select account_name, regional_office, count(*) as open_opportunity_count,
