...
These hooks keep domain-specific service behavior out of overwritten generated
files while preserving the same governed outcomes.
"""
...
def resolve_pipeline_scope(requested_scope: str | None, actor_policy: dict) -> str | None:
    ...
    if normalized_requested:
        lowered = normalized_requested.lower()
        for suffix in (" region", " territory", " office"):
            if lowered.endswith(suffix):
                normalized_requested = normalized_requested[: -len(suffix)].strip()
                break
