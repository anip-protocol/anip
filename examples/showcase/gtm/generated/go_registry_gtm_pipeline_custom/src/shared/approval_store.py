"""Generic file-backed approval store for showcase workflows."""
...
DEFAULT_APPROVAL_STORE_PATH = os.getenv(
    "GTM_APPROVAL_STORE_PATH",
    "/tmp/gtm-approvals.json",
)
...
def _store_path() -> Path:
    path = Path(DEFAULT_APPROVAL_STORE_PATH)
...
def get_approval_request(approval_request_id: str) -> dict[str, Any] | None:
    for item in list_approval_requests():
