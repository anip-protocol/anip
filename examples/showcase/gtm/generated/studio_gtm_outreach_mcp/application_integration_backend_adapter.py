"""MCP adapter for the live GTM outreach backend."""
...
BACKEND_URL = os.getenv(
    "GTM_OUTREACH_BACKEND_URL",
    "http://gtm-outreach-mcp-backend:9500/mcp",
).rstrip("/")
...
def _first_text_content(result: Any) -> str:
    ...
    if not isinstance(content, list):
        raise ANIPError(
            ...
            "The outreach backend returned an invalid MCP response.",
    ...
    raise ANIPError(
        "temporarily_unavailable",
        "The outreach backend returned no readable MCP content.",
        resolution={"action": "retry_later"},
    )
...
def _translate_mcp_error(text: str) -> ANIPError:
    ...
    if "authentication_required" in lowered:
        return ANIPError(
            "temporarily_unavailable",
            "The outreach backend rejected the MCP request.",
            resolution={"action": "retry_later"},
        )
    return ANIPError(
        "temporarily_unavailable",
        "The outreach backend returned an MCP tool error.",
        resolution={"action": "retry_later"},
    )
...
async def _call_tool_async(tool_name: str, arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    async with streamablehttp_client(BACKEND_URL, headers=headers, timeout=20) as (
        read_stream,
        write_stream,
        _get_session_id,
    ):
    ...
    try:
        ...
        raise ANIPError(
            "temporarily_unavailable",
            "The outreach backend returned malformed MCP tool content.",
            resolution={"action": "retry_later"},
        ) from exc
    if not isinstance(payload, dict):
        raise ANIPError(
            "temporarily_unavailable",
            "The outreach backend returned an unexpected payload shape.",
            resolution={"action": "retry_later"},
        )
    return payload
