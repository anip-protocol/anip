...
public class DelegationEngineTests : IDisposable
{
    ...
    [Fact]
    public void IssueToken_CallerSuppliedTaskId_EchoedInResponse()
    {
        var request = new TokenRequest
        {
            Subject = "agent@example.com",
            Scope = new List<string> { "data:read" },
            Capability = "summarize",
            PurposeParameters = new Dictionary<string, object> { ["task_id"] = "my-custom-task" }
        };

        var response = DelegationEngine.IssueDelegationToken(_keys, _storage, ServiceId, Principal, request);

        Assert.True(response.Issued);
        Assert.Equal("my-custom-task", response.TaskId);
    }

    [Fact]
    public void IssueToken_NoPurposeParams_AutoGeneratedTaskId()
    {
        var request = new TokenRequest
        {
            Subject = "agent@example.com",
            Scope = new List<string> { "data:read" },
            Capability = "summarize"
            // PurposeParameters is null (default)
        };
        ...
        Assert.StartsWith("task-", response.TaskId);
    }

    [Fact]
    public void IssueToken_PurposeParamsWithoutTaskId_ResponseOmitsTaskId()
    {
        var request = new TokenRequest
        {
            Subject = "agent@example.com",
            Scope = new List<string> { "data:read" },
            Capability = "summarize",
            PurposeParameters = new Dictionary<string, object> { ["source"] = "test" }
        };
