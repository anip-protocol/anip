...
export class DelegationEngine {
  ...
  private async _createToken(opts: {
    ...
    const token: DelegationTokenType = {
      ...
      caller_class: null,
      // v0.23: child tokens inherit parent.session_id verbatim; roots may
      // set it via opts. SPEC.md §4.8: session identity for session_bound
      // grants is bound to the signed token.
