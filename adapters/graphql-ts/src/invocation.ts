 ...
 * returning raw dicts for GraphQL serialization.
 */
...
export class ANIPInvoker {
  ...
  async setup(): Promise<void> {
    this.rootTokenId = `graphql-adapter-${randomHex(12)}`;
    ...
    const rootToken = {
      ...
      purpose: {
        capability: "*",
        parameters: {},
        task_id: `graphql-session-${randomHex(8)}`,
      },
      parent: null,
      expires,
      constraints: {
  ...
  async invoke(
    ...
    const capTokenId = `graphql-${capabilityName}-${randomHex(8)}`;
    const expires = new Date(
      Date.now() + this.config.tokenTtlMinutes * 60 * 1000
    ).toISOString();
    ...
    const capToken = {
      token_id: capTokenId,
      issuer: "adapter:anip-graphql-adapter-ts",
      subject: "adapter:anip-graphql-adapter-ts",
      scope: capScope,
      purpose: {
        capability: capabilityName,
        parameters: args,
        task_id: `graphql-invoke-${randomHex(8)}`,
      },
